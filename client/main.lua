...
AddEventHandler("medical:useItem", function(index)
    ...
    if not oncooldown then 
        local testplayer = exports["skrievo_minigame"]:taskBar(item.diff,7)
    ...
    while true do
        ...
        for k,v in pairs(Config.MedicLocations) do
            ...
            if distance <= 1.5 then
                ...
                if IsControlJustReleased(0, 0xD9D0E1C0) then
                    TriggerServerEvent('skrievo_medical:getjob', 'medic_house')
                end
            end
        end
        ...
        elseif WarMenu.IsMenuOpened('options_house') then
            if WarMenu.Button(_U('take_items')) then
                TriggerServerEvent('skrievo_medical:takeItems')
            end
            WarMenu.Display()
        ...
        elseif IsControlJustReleased(0, 5) then -- SUPR
            --TriggerServerEvent('skrievo_medical:getjob', 'medic')
        end
        Citizen.Wait(0)
    end
end)

RegisterNetEvent('skrievo_medical:auth')
AddEventHandler('skrievo_medical:auth', function(type)
    WarMenu.OpenMenu(type)
end)

RegisterNetEvent('skrievo_medical:open')
AddEventHandler('skrievo_medical:open', function(cb)
	WarMenu.OpenMenu('medic')
	--print ("openmenu")
end)


RegisterNetEvent('skrievo_medical:revive')
AddEventHandler('skrievo_medical:revive', function(closestPlayer)
    DoScreenFadeOut(800)
...
RegisterNetEvent('skrievo_medical:heal')
AddEventHandler('skrievo_medical:heal', function(closestPlayer)
