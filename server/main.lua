...
RegisterCommand('med', function(source, args, rawCommand)
    ...
    if job == 'doctor' then
        TriggerClientEvent('skrievo_medical:open', _source)
    else
...
RegisterServerEvent('skrievo_medical:getjob')
AddEventHandler('skrievo_medical:getjob', function(type)
    local _source = source
    local Character = VorpCore.getUser(_source).getUsedCharacter
    local job = Character.job
    if job == 'doctor' then
        TriggerClientEvent('skrievo_medical:auth', _source, type)
    else
        TriggerClientEvent("vorp:TipBottom", source, 'You need a Medical Certificate', 6000)    
    end
end)

RegisterServerEvent('skrievo_medical:reviveplayer')
AddEventHandler('skrievo_medical:reviveplayer', function(closestPlayer)
    local _source = source
    local Character = VorpCore.getUser(_source).getUsedCharacter
    local count = Inventory.getItemCount(_source, "syringe")
    local job = Character.job
    if job == "police" or job == "marshal" then 
        TriggerClientEvent('skrievo_medical:revive', closestPlayer)
    end
...
RegisterServerEvent('skrievo_medical:healplayer')
AddEventHandler('skrievo_medical:healplayer', function(closestPlayer)
    local _source = source
    local Character = VorpCore.getUser(_source).getUsedCharacter
    ...
    if job == 'doctor' and count > 0 then
        ...
        TriggerClientEvent('skrievo_medical:heal', closestPlayer)
    else
